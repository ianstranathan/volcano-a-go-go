...
# Music functions
# ------------------------------------------------------------------------------
func play_music(track_id: int) -> void:
...
func _play_world_sound_on_voice(
	...
	var bus: StringName = sound_def.get("bus", &"Effects")
	var default_volume_db: float = sound_def.get("default_volume_db", 0.0)
	var default_pitch_scale: float = sound_def.get("default_pitch_scale", 1.0)
	...
	voice.bus = bus
	voice.volume_db = default_volume_db + adjustments.get("volume_db", 0.0)
	voice.pitch_scale = default_pitch_scale * adjustments.get("pitch_scale", 1.0)
	voice.max_distance = sound_def.get("max_distance", 1200.0)
	voice.attenuation = sound_def.get("attenuation", 1.0)
	voice.panning_strength = sound_def.get("panning_strength", 1.0)
...
func play_world_sound_local(sound_id: int, world_position: Vector2, adjustments: Dictionary = {}) -> void:
...
func play_world_sound(sound_id: int, world_position: Vector2, adjustments: Dictionary = {}) -> void:
	play_world_sound_local(sound_id, world_position, adjustments)
	
	if multiplayer.multiplayer_peer == null:
		return
	
	play_world_sound_remote.rpc(sound_id, world_position, adjustments)
	
@rpc("any_peer", "call_remote", "unreliable")
func play_world_sound_remote(sound_id: int, world_position: Vector2, adjustments: Dictionary = {}) -> void:
	play_world_sound_local(sound_id, world_position, adjustments)
